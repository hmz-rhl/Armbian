...
InstallHubload(){
	...
	sed -i '$a earlycon=on' /boot/armbianEnv.txt
	# on modifie notre environnement de boot pour augmenter le buffer de spidev utilisé par les leds
	sed -i '$a extraargs=spidev.bufsiz=65534' /boot/armbianEnv.txt
	...
	export LANG=C LC_ALL="us_US.UTF-8"
	...
	apt-get update
	apt-get --yes install libnfc6 libnfc-bin libnfc-dev libnfc-examples libnfc-pn53x-examples
	apt-get --yes install libgpiod-dev gpiod
	apt-get --yes install swupdate
	apt-get --yes install openjdk-17-jdk
	apt-get --yes install libsdl2*
	apt-get --yes install xinit imagemagick xorg libgtk2.0-0 x11-*
	apt-get --yes install libgl1-mesa-dev libglu1-mesa* libegl1-mesa*
	apt-get --yes install i2c-tools
	apt-get --yes install strace

#eeprom voir comment l'installer ensuite
	wget https://www.codesink.org/download/eeprog-0.7.6.tar.gz
	wget https://github.com/tat/eeprog/archive/refs/heads/master.zip

#peut etre mettre ca dans un fichiers dans /etc/default/hubload/charger.properties
	cat > /root/charger.properties <<- EOF
	cp.identifier=HubLoadv3
	cp.model=HL-V3-22kW
	cp.vendor=Electric Loading
	cp.firmware.version=0.101
	cp.iccid=
	cp.imsi=
	cp.meter.serial=XXXXXXX*XXXXXXX
	cp.meter.type=
	cp.serial=SAEM*HUBV3*00010001
	cp.profiles=standard
	conn.type=Type2
	conn.price=0.5
	conn.intensity=16
	conn.phases=1
	ocpp.server=
	ocpp.port=8887
	maintenance.code=654321
	EOF


	#cd /home/rpi-software-suite/src/c/
       	#./install.sh
	#cd -
	#chroot $SDCARD

#Remove annoying user creation at login
	rm -f /root/.not_logged_in_yet
	echo -e "password\npassword" | (passwd root)

#on restaure le resolv.conf
	##rm /etc/resolv.conf
